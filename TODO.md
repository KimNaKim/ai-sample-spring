...
- [ ] T-1.1 `data.sql`에 20개 이상의 게시글 데이터 추가 (학습용 환경 구축)
- [ ] T-1.2 페이징 없는 전체 게시글 목록 보기 기능 구현 (`BoardService.게시글목록보기`)
- [ ] T-1.3 `list.mustache` 생성 및 전체 목록 출력 확인
...
- [ ] T-5.2 현재 페이지 강조(Active) 및 사용자 친화적 번호(1부터 시작) 구현 (UX 개선)
- [ ] T-5.3 라이브러리 없이 구현한 페이징 핵심 원리 복기 및 최종 정리
